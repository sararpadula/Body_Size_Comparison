...
## Model first egg ######
fe1 <- lmer(First.Egg ~ Tarsus_Difference*Species + Species*scale_ele + (1|Year), data = breed.fe)
fe2 <- lmer(First.Egg ~ Tarsus_Difference*Species + scale_ele + (1|Year), data = breed.fe)
...
range(mochBreed$Female.Wing.Chord)
#plotting the quadratic model
fwingvalues <- seq(62,71,0.1)

predictedValue <- predict(quadraticMoch,list(Female.Wing.Chord = fwingvalues, Female.Wing.Chord2 = fwingvalues^2))

plot(mochBreed$Female.Wing.Chord, mochBreed$Male.Wing.Chord, pch=16, xlab="Female Wing Chord (mm)", ylab="Male Wing Chord (mm)", cex.lab=1.3)
